"""Train and compare Task 2 forecasting models for Horizon 12 months.

Horizon-specific defaults:
  - Lookback : 12 months  (start here; also try l18 / l24 via --data flag)
  - BiLSTM   : 128 → 64 units (bidirectional)
  - Dense    : 32 → 16 → 1 (three-layer head for added capacity)
  - Epochs   : 100
  - Batch    : 32
  - ES pat.  : 15
"""
...
HORIZON = 12
DEFAULT_LOOKBACK = 12
DEFAULT_LSTM_UNITS_1 = 128
DEFAULT_LSTM_UNITS_2 = 64
DEFAULT_DENSE_UNITS = [32, 16]      # Dense(32) → Dense(16) → Dense(1)
DEFAULT_DROPOUT = 0.2
DEFAULT_EPOCHS = 100
DEFAULT_BATCH = 32
DEFAULT_PATIENCE = 15
...
# ---------------------------------------------------------------------------
# LSTM  (same architecture as H3/H6/H9 — kept stable for fair comparison)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# BiLSTM + Attention  (H12-tuned: 128→64, three-layer dense head)
# ---------------------------------------------------------------------------
...
def build_bilstm_attention_model(
    ...
    H12 defaults: 128 → 64 BiLSTM units, Dense(32) → Dense(16) → Dense(1).
    Larger capacity than H9 to handle the longer forecast difficulty.
    """
    ...
    # Bidirectional encoder — wider than H9
    ...
    # Three-layer dense head
    for units in dense_units:
        x = layers.Dense(units, activation="relu")(x)
    outputs = layers.Dense(1)(x)

    model = tf.keras.Model(inputs=inputs, outputs=outputs, name="bilstm_attention_h12")
    model.compile(
        optimizer=tf.keras.optimizers.Adam(learning_rate=learning_rate),
        loss="mse",
        metrics=[tf.keras.metrics.MeanAbsoluteError(name="mae")],
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train Task 2 models for Horizon 12 months."
    )
    parser.add_argument(
        "--data",
        type=Path,
        default=Path("task2_processed/task2_processed_data_l12_h12.csv"),
        help=(
            "Pre-built data file from prepare_task2_data.py. "
            "Swap to l18_h12 or l24_h12 to test other lookbacks."
        ),
    )
    ...
    parser.add_argument(
        "--metrics-output",
        type=Path,
        default=Path("task2_output/task2_model_metrics_l12_h12.json"),
    )
    parser.add_argument(
        "--predictions-output",
        type=Path,
        default=Path("task2_output/task2_model_predictions_l12_h12.csv"),
    )
    parser.add_argument(
        "--model-output",
        type=Path,
        default=Path("bilstm_attention_results/bilstm_attention_model_h12.keras"),
    )
...
def main() -> None:
    ...
    # Infer lookback from filename for logging (e.g. "l18_h12" → 18)
    stem = args.data.stem
    lookback_label = DEFAULT_LOOKBACK
    for part in stem.split("_"):
        if part.startswith("l") and part[1:].isdigit():
            lookback_label = int(part[1:])

    print(f"[H12] Loading data from: {args.data}")
    ...
    for split, mask in [("train", train_mask), ("val", val_mask), ("test", test_mask)]:
        if mask.sum() == 0:
            raise ValueError(f"[H12] {split} split is empty — check data file generation.")
    ...
    # ---- BiLSTM + Attention ------------------------------------------------
    print("\nTraining BiLSTM+Attention (H12: 128→64 units, dense [32, 16])...")
    try:
    ...
    # ---- Summary -----------------------------------------------------------
    print("\n" + "=" * 80)
    print(f"TASK 2 MODEL COMPARISON  [Horizon = {HORIZON} months | Lookback = {lookback_label} months]")
    print("=" * 80)
    ...
    args.metrics_output.write_text(
        json.dumps({
            ...
            "lookback_months": lookback_label,
